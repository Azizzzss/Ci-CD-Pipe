...
@Service
public class LivrePaieRepService {
    ...
    public LivrePaieRepService(IdtagentRepository idtagentRepository, LivrePaieService livrePaieService) {
    ...
    public byte[] generateLivrePaieRep(Integer p_annee, Integer p_mois,  String p_ufg) throws JRException {

        List<LivrePaieDTO> livrePaieDTOList = idtagentRepository.getLivrePaie(p_annee,p_mois,p_ufg);

        log.info("get list with params  {} {} {}  ", p_annee , p_mois , p_ufg);

        log.info(" size list is {}  ",livrePaieDTOList.size());


        if (!livrePaieDTOList.isEmpty()) {

            log.info(" ------------- size is {}   ",livrePaieDTOList.size());


            //log.info("DifNetDTO list found");
            String templatePath = "livre_paie";

            Map<String, Object> parametres = new HashMap<>();
            parametres.put("p_annee", p_annee);
            parametres.put("p_mois", p_mois);
            parametres.put("p_ufg", p_ufg);
            parametres.put("FileTitle", "Livre De Paie");

            JasperReport subReport = JasperCompileManager.compileReport(this.getClass().getResourceAsStream("/tunisair_rep/sub_livre_paie.jrxml"));
            JRSaver.saveObject(subReport, "d:\\sub_livre_paie.jasper");

            List<LivrePaie> livrePaies = livrePaieService.getLivrePaie( p_annee, p_mois, p_ufg);

            JRBeanCollectionDataSource source = new JRBeanCollectionDataSource(livrePaies);

            parametres.put("livrepaiesource", source);
