...
@Service
public class DecImpoRepService {

    Logger log = LoggerFactory.getLogger(DecImpoRepService.class);


    private final Dec_ImpoService decImpoService ;


    public DecImpoRepService(Dec_ImpoService decImpoService) {
        this.decImpoService = decImpoService;
    }
    ...
    public byte[] generateDecimpoRep(Integer p_annee, Integer p_mois) throws JRException {

        List<Dec_impo> Decassurlist = decImpoService.getDecimpo(p_annee,p_mois);

        if (!Decassurlist.isEmpty()) {

            //log.info("DifNetDTO list found");
            String templatePath = "dec_impo";

            Map<String, Object> parametres = new HashMap<>();
            parametres.put("p_annee", p_annee);
            parametres.put("p_mois", p_mois);
            parametres.put("FileTitle", "Dec_impo");


            JasperReport subReport = JasperCompileManager.compileReport(this.getClass().getResourceAsStream("/tunisair_rep/sub_dec_impo.jrxml"));
            JRSaver.saveObject(subReport, "d:\\sub_dec_impo.jasper");

            JRBeanCollectionDataSource source = new JRBeanCollectionDataSource(Decassurlist);

            parametres.put("decimporesource", source);
